	...
	*log_level_ret = level;
	...
	do {
		...
		for (Py_ssize_t i = 0; i < size; i++) {
			...
			_cleanup_pydecref_ PyObject *fileno =
				PyObject_GetAttrString(stream, "fileno");
			if (!fileno) {
				PyObject *exc_type, *exc_value, *exc_traceback;
				PyErr_Fetch(&exc_type, &exc_value,
					    &exc_traceback);
				if (PyErr_GivenExceptionMatches(exc_type,
					...
					PyErr_Restore(exc_type, exc_value,
						      exc_traceback);
					return -1;
				}
			}
